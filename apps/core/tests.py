...
class AdminDashboardTests(TestCase):
    ...
    def setUp(self):
        ...
        self.creation_user = User.objects.create_user(
            username="creator", password="testpass123", is_staff=False
        )
        self.explore_user = User.objects.create_user(
            username="explorer", password="testpass123", is_staff=False
        )
    ...
    def test_admin_dashboard_requires_admin_permission(self):
        """Test that admin dashboard requires admin user type"""
        # Try with explore user - should be denied access
        self.client.login(username="explorer", password="testpass123")
        response = self.client.get(self.url)
        # Should either return 403 or redirect (both are valid)
        self.assertIn(response.status_code, [302, 403])

        # Try with creation user - should be denied access
        self.client.logout()
    ...
    def test_admin_dashboard_loads_for_admin(self):
        """Test that admin dashboard loads successfully for admin users"""
